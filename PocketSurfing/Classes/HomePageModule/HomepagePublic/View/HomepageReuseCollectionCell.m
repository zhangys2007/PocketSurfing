...
@implementation HomepageReuseCollectionCell
- (void)dealloc
{
    self.imageView = nil;
    self.titleLabel = nil;
...
//        [imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",self.picURL]]];
        ...
        [similarBtn addTarget:self action:@selector(likeBtn:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)setDataModel:(HomePageReuseCellData *)dataModel
{
    if (_dataModel != dataModel) {
        ...
        _likeCountLabel.text = _dataModel.total_love_number;
